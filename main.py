...
# Inicializamos el cliente para BOT
...
def crear_boton_whatsapp():
    ...
    # Usamos Button.url que SÍ acepta enlaces externos
    return [[Button.url("📱 Contactar por WhatsApp", url)]]
...
async def enviar_galeria(event):
    ...
    await event.respond("📸 **Aquí te comparto la galería del apartamento:**", buttons=crear_boton_whatsapp())
    
    for img_path in imagenes:
        if os.path.exists(img_path):
            try:
                await event.respond(file=img_path)
                await asyncio.sleep(1.5)  # Pausa para evitar límites de Telegram
            except Exception as e:
                print(f"⚠️ Error enviando {img_path}: {e}")
        else:
            print(f"⚠️ Imagen no encontrada en carpeta: {img_path}")
            
    await event.respond("📱 **¿Te gustó?** Escríbenos al WhatsApp para reservar.", buttons=crear_boton_whatsapp())
...
async def handler(event):
    ...
    if mensaje in ['/start', 'hola', 'buenas', 'inicio', '']:
        await event.respond(
            ...
            buttons=crear_boton_whatsapp()
        )
    
    elif mensaje in ['/info', 'informacion', 'información', 'detalles']:
        await event.respond(INFO_APARTAMENTO, parse_mode='markdown', buttons=crear_boton_whatsapp())
